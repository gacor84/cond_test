...
### [Contingency table](http://en.wikipedia.org/wiki/Sensitivity_and_specificity):   
$$\\
\\$$
...
- ***Test outcome* conditioned on *condition***
...
Probability of $-$ given $D \rightarrow p(-|D)=\frac{p(- \cap D)}{p(D)}=\frac{\text{FN}}{\text{TP+FN}} \Rightarrow$ **FALSE NEGATIVE RATE**
...
$\text{SENSITIVITY}:p(+|D)=\frac{p(+ \cap D)}{p(D)}$, $\ \text{PPV}:p(D|+)=\frac{p(+ \cap D)}{p(+)}$

$\rightarrow p(+ \cap D)=p(+|D)p(D)=p(D|+)p(+) \ \equiv \ \text{TP}=\text{SENSITIVITY}*\text{PREVALENCE}=\text{PPV}*\text{(TP+FP)}$
...
$p(D|+)=\frac{p(+|D)p(D)}{p(+|D)p(D)+p(+|D^c)p(D^c)} \ \equiv \ \text{PPV}=\frac{\text{SENSITIVITY*PREVALENCE}}{\text{SENSITIVITY*PREVALENCE}+(1-\text{SPECIFICITY})*(1-\text{PREVALENCE})}$
...
$\text{PPV} = p(D|+) = \frac{p(+|D)p(D)}{p(+|D)p(D)+p(+|D^c)p(D^c)}$, $\ \text{FDR} = p(D^c|+) = \frac{p(+|D^c)p(D^c)}{p(+|D)p(D)+p(+|D^c)p(D^c)}$
$\\$

$\rightarrow \frac{p(D|+)}{p(D^c|+)} = \frac{p(+|D)}{p(+|D^c)} * \frac{P(D)}{P(D^c)} \ \equiv \ \text{POST TEST ODDS} = \text{DLR}_{+} * \text{PRE TEST ODDS} \Rightarrow$

$\Rightarrow \text{DLR}_{+} = \frac{\text{POST TEST ODDS}}{\text{PRE TEST ODDS}}$
$\\$
...
### [Type I and II errors](http://es.wikipedia.org/wiki/Errores_de_tipo_I_y_de_tipo_II)
