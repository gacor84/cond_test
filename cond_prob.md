...
$\rightarrow p(+ \cap D)=p(+|D)p(D)=p(D|+)p(+)$ 

$\equiv \text{TP}=\text{SENSITIVITY}*\text{PREVALENCE}=\text{PPV}*\text{(TP+FP)}$
...
$\equiv p(+)=p(+|D)p(D)+p(+|D^c)p(D^c)=p(+ \cap D)+p(+ \cap D^c)$

$\rightarrow \text{TP+FP}=\text{TP+FP}$

---

$p(D|+)=\frac{p(+|D)p(D)}{p(+|D)p(D)+p(+|D^c)p(D^c)}$

$\equiv \text{PPV}=\frac{\text{SENSITIVITY*PREVALENCE}}{\text{SENSITIVITY*PREVALENCE}+(1-\text{SPECIFICITY})*(1-\text{PREVALENCE})}$
